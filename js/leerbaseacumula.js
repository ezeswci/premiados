...
function querySuccessAcum(tx, rs) {
    ...
    //for (var i = 0; i < rs.rows.length; i++) {
        var p = rs.rows.item(0);
		window.monedasAcum=p.ro_monedas;
		cargarMonedasAcum();
		//alert("monedas");
    //}
}
function cargarMonedasAcum(){
	if(document.getElementById("mon_ac")!=null){
	document.getElementById("mon_ac").innerHTML=window.monedasAcum;}else{setTimeout(function(){cargarMonedasAcum();},50)
	}
}